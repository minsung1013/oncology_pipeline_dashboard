...
import { buildBiomarkerOpportunityRows } from '../utils/biomarkerAggregations'
import {
  applyRecencyWeight, flagEmerging, EMERGE_DEFAULTS, HALFLIFE_DEFAULT, CLIN_AXIS, deriveYearWindows,
...
export default function BiomarkerOpportunityPage() {
  ...
  function setThr(patch) {
    setThrState((prev) => {
      ...
      setTabState('bioOpportunity', { ...getTabState('bioOpportunity'), thr: next })
      return next
    })
  }
  ...
  const baseRows = useMemo(
    () => buildBiomarkerOpportunityRows(drugs, filteredAbstracts),
    [drugs, filteredAbstracts],
  )
  const maxYear = useMemo(() => deriveYearWindows(filteredAbstracts).maxYear, [filteredAbstracts])
  const weighted = useMemo(() => applyRecencyWeight(baseRows, { halfLife, maxYear }), [baseRows, halfLife, maxYear])
  ...
  const summary = `${emerging.length} emerging · ${rows.filter((r) => r.pre_weighted > 0 || r.clinical_total > 0).length} biomarkers`
    ...
    <div className="flex flex-col h-full">
      <div className="bg-white border-b border-slate-200 px-4 py-3 shrink-0">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h2 className="text-base font-bold text-slate-800 leading-none">Biomarker Opportunity Map</h2>
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        <p className="text-xs text-slate-500 leading-relaxed">
          ...
          <b className="text-red-600"> 빨강 테두리=부상</b> · 🆕=최근 첫 등장. 왼쪽 위 핑크 = 화이트스페이스. 점/행 클릭 시 해당 바이오마커로 필터.
        </p>

        <TargetOpportunityMap rows={rows} selected={filters.biomarkers} onSelect={toggleBiomarker} />

        <div>
          <h3 className="text-sm font-bold text-slate-700 mb-1">부상 바이오마커 <span className="text-slate-400 font-normal">({emerging.length}) — 연구 활발 · 임상 초기 · 상승</span></h3>
          <div className="overflow-auto border border-slate-200 rounded">
            <table className="w-full text-xs">
              ...
              <tbody>
                {emerging.map((r) => {
                  const sel = filters.biomarkers.includes(r.target)
                  return (
                    <tr key={r.target} onClick={() => toggleBiomarker(r.target)}
                ...
                {emerging.length === 0 && (
                  <tr><td colSpan="11" className="px-2 py-4 text-center text-slate-400">조건에 맞는 부상 바이오마커 없음 — 임계값을 완화하세요.</td></tr>
                )}
              </tbody>
            </table>
          </div>
