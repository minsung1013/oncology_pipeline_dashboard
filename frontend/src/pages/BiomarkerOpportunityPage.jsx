...
import {
  applyRecency, flagEmerging, EMERGE_DEFAULTS, HALFLIFE_DEFAULT,
} from '../utils/opportunityAggregations'
import { getShared, setShared, getTabState, setTabState } from '../utils/filterStore'
import {
  getPipeline, getAbstractIndex, loadAbstractFiles, getPublicationIndex, loadPublicationFiles,
...
export default function BiomarkerOpportunityPage() {
  ...
  const baseRows = useMemo(
    () => buildBiomarkerOpportunityRows(drugs, filteredAbstracts, filteredPubs),
  ...
  const summary = `${emerging.length} emerging · ${rows.filter((r) => r.size_total > 0).length} biomarkers`
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        <p className="text-xs text-slate-500 leading-relaxed">
          <b>x</b>=임상 성숙도(단계 가중 0.3~4 × 진행상태[완료↑·중단↓]의 <b>총합, 로그 스케일</b>) · <b>y</b>=<b>최신 임팩트</b>(임상·초록·논문 발표의 최근가중 합, 로그; 최근·다수일수록 상단) ·
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-700 mb-1">부상 바이오마커 <span className="text-slate-400 font-normal">({emerging.length}) — 최신·상승 · 임상 미성숙</span></h3>
          <div className="overflow-auto border border-slate-200 rounded">
            <table className="w-full text-xs">
              ...
              <tbody>
                ...
                {emerging.length === 0 && (
                  <tr><td colSpan="10" className="px-2 py-4 text-center text-slate-400">조건에 맞는 부상 바이오마커 없음 — 임계값을 완화하세요.</td></tr>
                )}
              </tbody>
            </table>
          </div>
