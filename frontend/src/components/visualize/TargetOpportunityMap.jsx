...
// 기회 지도 산점도 — 두 지표를 축에 배치(단계 가중 없음).
//   x = 임상 성숙도(clin_maturity_idx 0..6) · y = 전임상 최신가중 연구강도(pre_weighted, log)
//   색 = 성장비(파랑=식음→빨강=뜸) · 크기 = 고유 기관 수 · 빨강 테두리 = 부상
// rows: applyRecencyWeight + flagEmerging 적용된 행. selected: canonical target 배열. onSelect(target).
...
export default function TargetOpportunityMap({ rows, selected = [], onSelect }) {
  ...
  const pts = useMemo(
    () => rows.filter((r) => r.pre_weighted > 0 || r.clinical_total > 0),
    [rows],
  )
  const ymax = useMemo(() => Math.max(1, ...pts.map((r) => r.pre_weighted)), [pts])
  const xmax = 6

  const X = (idx, jit = 0) => padL + ((idx + jit) / xmax) * plotW
  const Y = (n) => padT + plotH - (Math.log1p(n) / Math.log1p(ymax)) * plotH
  const R = (n) => 3 + Math.sqrt(n) * 1.1 // 크기 = 고유 기관 수

  // 라벨 대상: 활동량 상위 26 + 부상 전부 + 선택 전부
  const labeled = useMemo(() => {
    const top = [...pts].sort(
      (a, b) => b.pre_weighted + b.clinical_total - (a.pre_weighted + a.clinical_total),
    ).slice(0, 26)
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto select-none" style={{ maxHeight: '72vh' }}>
      ...
      {/* 격자 (y) */}
      {[1, 3, 10, 30, 100, 300].filter((n) => n <= ymax).map((n) => (
        <g key={n}>
      ...
      <text x="22" y={padT + plotH / 2} fontSize="12" fontWeight="600" textAnchor="middle" fill="#444" transform={`rotate(-90 22 ${padT + plotH / 2})`}>전임상 연구 강도 (최신 가중 초록, log) →</text>

      {/* 점 */}
      {pts.map((r) => {
        const x = X(r.clin_maturity_idx, hashJit(r.target))
        const y = Y(r.pre_weighted)
        const rr = R(r.pre_orgs)
        const sel = selSet.has(r.target)
        const stroke = sel ? '#1d4ed8' : r.emerging ? '#c10000' : '#8894a4'
        const sw = sel ? 2.6 : r.emerging ? 2.2 : 0.6
        const tip =
          `${r.target} — 최신가중 ${r.pre_weighted} · 기관 ${r.pre_orgs} · 초록 ${r.pre_abstracts} · 모달 ${r.pre_modalities}` +
